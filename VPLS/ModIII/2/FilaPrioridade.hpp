...
public:
  ...
  /** Essa função remove o primeiro nó da lista (pois é o de maior prioridade (menor inteiro)), e retorna o valor do dado armazenado no nó.
       @retorna a prioridade do pFrimeiro nó da lista.
     */
  int remover();
...
private:
  ...
  No *fim;
  int tamanho;
  void inserirNo(No *atual, No *inserido);
  void contarTamanho(No *proximo);
  /* No *encontrarNo(No *proximo, int *dado); */
  No *encontrarMeio(No *proximo, unsigned *count);
  No *getNoDado(int dado);
  void printFila(No *atual);
};
